...
첫 탐구(그림자)는 자람마을 백엔드(`Think-Forest-Backend`)의 사고력 엔진과 연결됩니다.

## 지금까지 진행한 내용 (2026-09-13 기준)

| 영역                  | 내용                                                                                                                               | 상태                       |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| 기본 화면             | 오늘의 모험, 시작 안내·진단, 생각숲·실험실·마음극장, 책장, 리포트, 보호자 화면                                                     | ✅                         |
| API 연결층            | API 클라이언트 주입 구조, 타입, 도메인 요청 함수, TanStack Query 훅                                                                | ✅                         |
| 첫 탐구 v2            | **헷갈리는 생각 친구 가르치기** — 예측·확신도 → 친구 생각 → 공정한 실험 설계 → 증거로 설득 → 새 예측 검사 → 사고 기술              | ✅ 로컬 완주 확인          |
...
첫 탐구는 백엔드가 있어야 진행됩니다. `Think-Forest-Backend` README의 빠른 시작을 따릅니다.
...
백엔드 `.env`의 `CORS_ORIGINS`에 `http://127.0.0.1:5173,http://localhost:5173`을 넣습니다.
...
- 백엔드 주소는 `VITE_API_BASE_URL`로 넣습니다. 매번 적기 싫으면 `apps/web/.env`에
  `VITE_API_BASE_URL=http://127.0.0.1:8010`을 적습니다(`apps/web/.env.example` 참고).
